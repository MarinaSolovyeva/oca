package org.oca.test3;

/**
 * What is the output of the following code? (Choose all that apply)
 * 1: interface HasTail { int getTailLength(); }
 * 2: abstract class Puma implements HasTail {
 * 3: protected int getTailLength() {return 4;}
 * 4: }
 * 5: public class Cougar extends Puma {
 * 6: public static void main(String[] args) {
 * 7: Puma puma = new Puma();
 * 8: System.out.println(puma.getTailLength());
 * 9: }
 * 10:
 * 11: public int getTailLength(int length) {return 2;}
 * 12: }
 * A. 2
 * B. 4
 * C. The code will not compile because of line 3.  // true - 'getTailLength()' in 'org.oca.test3.Puma' clashes with 'getTailLength()' in 'org.oca.test3.HasTail'; attempting to assign weaker access privileges ('protected'); was 'public'
 * D. The code will not compile because of line 5.
 * E. The code will not compile because of line 7.
 * F. The code will not compile because of line 11.
 * G. The output cannot be determined from the code provided.
 */
interface HasTail {
    int getTailLength();
}

abstract class Puma implements HasTail {
    protected int getTailLength() {
        return 4;
    }
}

public class Cougar extends Puma {
    public static void main(String[] args) {
        Puma puma = new Puma();
        System.out.println(puma.getTailLength());
    }

    public int getTailLength(int length) {
        return 2;
    }
}
